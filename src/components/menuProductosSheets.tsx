...
export default function MenuProductosSheets() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const sheetId = "1ds8bBZaNd1b05_WfK7ROm4w6-672LOe1muMjfQqz9uk";
                const range = "MenuCode!A:E";
                const apiKey = "AIzaSyCzk-72OcOPucXDYHx9Xk9FMgBJ49XBacs";
        ...
        <section className="menuSheetsProductos">
            ...
            <div className="menuSheetsProductos-cards">
                {filtrados.map((p) => (
                    <div
                        key={p.id}
                        className="menuSheetsProductos-card"
                        onClick={() => setProductoSeleccionado(p)}
                    >
                        <div className="menuSheetsProductos-imagen">
                            <img
                                src={
                                    !p.img || p.img === "indefinido" || p.img === "#N/A"
                                        ? "/images/productoDefault.png"
                                        : p.img.trim()
                                }
                                alt={p.nombre}
            ...
            {/* Modal responsivo con descripción */}
            {productoSeleccionado && (
                <div className="modal" onClick={() => setProductoSeleccionado(null)}>
                    <div className="modal-contenido">
                        <img
                            src={
                                !productoSeleccionado.img ||
                                productoSeleccionado.img === "indefinido" ||
                                productoSeleccionado.img === "#N/A"
                                    ? "/images/productoDefault.png"
                                    : productoSeleccionado.img.trim()
                            }
                            alt={productoSeleccionado.nombre}
                        />
                        <h3>{productoSeleccionado.nombre}</h3>
                        <p>{productoSeleccionado.descripcion}</p>
                        <span className="modal-precio">{productoSeleccionado.precio}</span>
                    </div>
                </div>
            )}
        </section>
